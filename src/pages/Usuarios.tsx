...
import { Usuario, useUsuarios, useEmpresas } from "@/services";
...
import {
  addEmpresaToCarteira,
  getAllCarteiraMap,
  removeEmpresaFromCarteira,
} from "@/services/useCarteira";
...
export default function Usuarios() {
  ...
  const [usuariosTab, setUsuariosTab] = useState<"usuarios" | "carteiras">("usuarios");
  const [carteiraMap, setCarteiraMap] = useState(() => getAllCarteiraMap());
  ...
  const handleAddToCarteira = (usuarioId: number, empresaId: number) => {
    addEmpresaToCarteira(usuarioId, empresaId);
    setCarteiraMap(getAllCarteiraMap());
  };

  const handleRemoveFromCarteira = (usuarioId: number, empresaId: number) => {
    removeEmpresaFromCarteira(usuarioId, empresaId);
    setCarteiraMap(getAllCarteiraMap());
  };
  ...
  const analistas = useMemo(
    () =>
      usuarios.filter(
        (u) =>
          (u.cargoNome ?? "").toUpperCase().includes("ANALISTA") ||
          (u.cargo ?? "").toUpperCase().includes("ANALISTA")
      ),
    [usuarios]
  );
    ...
    <div className="relative min-h-screen overflow-hidden bg-background text-foreground transition-colors duration-500">
      ...
      <div className="relative z-10 flex min-h-screen">
        ...
        <main className={`flex-1 transition-all duration-300 ${sidebarCollapsed ? "ml-[72px]" : "ml-[220px]"}`}>
          ...
          {currentRole === "ADMIN" && (
            <div className="px-6 pb-3 pt-2 flex gap-1">
              <div className="flex gap-1 p-1 bg-muted/20 rounded-lg border border-border/20">
                {(["usuarios", "carteiras"] as const).map((tab) => (
                  <button
                    key={tab}
                  ...
                  >
                    {tab === "usuarios" ? "Todos os Usuários" : "Analistas e Carteiras"}
                  </button>
                ))}
              </div>
            </div>
          ...
          {usuariosTab === "carteiras" && currentRole === "ADMIN" ? (
            <div className="px-6 pb-6 grid gap-4 md:grid-cols-2">
              {analistas.length === 0 ? (
                <div className="col-span-2 py-12 text-center text-[12px] text-muted-foreground/40">
                  Nenhum analista encontrado.
                </div>
              ) : (
                analistas.map((analyst) => {
                  const allocatedIds: number[] = carteiraMap[analyst.id] ?? [];
                  const allocatedCompanies = empresas.filter((emp) => allocatedIds.includes(emp.id));
                  return (
                    <div key={analyst.id} className="rounded-xl border border-border/25 bg-card/40 p-5">
                      <div className="flex items-center justify-between mb-3">
                        <div>
                          <p className="text-[13px] font-semibold">{analyst.nomeCompleto}</p>
                          <p className="text-[11px] text-muted-foreground/50">{analyst.email}</p>
                        </div>
                        <span className="text-[10px] px-2 py-0.5 rounded-full bg-accent/10 text-accent border border-accent/15">
                          {analyst.cargoNome || analyst.cargo}
                        </span>
                      </div>
                      <div className="flex flex-wrap gap-2 min-h-[28px]">
                        {allocatedCompanies.map((emp) => (
                          <span
                            key={emp.id}
                            className="flex items-center gap-1 text-[11px] px-2 py-0.5 rounded-full bg-muted/30 border border-border/20"
                          >
                            {emp.nome}
                            <button
                              onClick={() => handleRemoveFromCarteira(analyst.id, emp.id)}
                        ...
                        {allocatedCompanies.length === 0 && (
                          <span className="text-[11px] text-muted-foreground/30">Nenhuma empresa alocada</span>
                        )}
                      </div>
                      <div className="mt-3">
                        <select
                          onChange={(e) => {
                            if (e.target.value) {
                              handleAddToCarteira(analyst.id, Number(e.target.value));
                              e.target.value = "";
                            }
                          }}
                          className="text-[11px] h-7 px-2 rounded-lg border border-border/25 bg-card/40 text-muted-foreground"
                        >
                          <option value="">+ Alocar empresa...</option>
                          {empresas
                            .filter((emp) => !allocatedIds.includes(emp.id))
                            .map((emp) => (
